...
object Firework:
  ...
  //directly call pass inside case in every case class
...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      copy(countDown = countDown -1, position = Motion.movePoint(position,direction, speed = Settings.propulsionSpeed)) //revisar aquí
